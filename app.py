...
from PySide6.QtCore import QProcess, QRegularExpression, QSize, QRect, Qt
...
class PythonHighlighter(QSyntaxHighlighter):
    def __init__(self, parent):
        ...
        self.string_format.setForeground(QColor("#d69d85"))
    ...
    def highlightBlock(self, text: str):
        ...
        match = string_pattern.match(text)
        while match.hasMatch():
            self.setFormat(match.capturedStart(), match.capturedLength(), self.string_format)
            match = string_pattern.match(text, match.capturedEnd())
        ...
        while match.hasMatch():
            ...
            match = comment_pattern.match(text, match.capturedEnd())
        ...
        while match.hasMatch():
            ...
            match = number_pattern.match(text, match.capturedEnd())
...
class PythonIDE(QMainWindow):
    ...
    def new_file(self):
        ...
        self.file_label.setText("Untitled")
        self.setWindowTitle("Python IDE - New File")
        self.console_clear()

    def open_folder_dialog(self):
        folder = QFileDialog.getExistingDirectory(self, "Open Project Folder", str(self.project_root))
        if folder:
    ...
    def open_file_dialog(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Open Python File", str(self.project_root), "Python Files (*.py);;All Files (*)")
        if file_path:
    ...
    def open_file(self, file_path: Path):
        ...
        try:
            ...
            self.setWindowTitle(f"Python IDE - {file_path.name}")
    ...
    def save_file_as(self):
        ...
        if editor is None:
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Python File", str(self.project_root / "untitled.py"), "Python Files (*.py);;All Files (*)")
        if file_path:
            path = Path(file_path)
            ...
            self.setWindowTitle(f"Python IDE - {path.name}")
    ...
    def choose_python_interpreter(self):
        python_path, _ = QFileDialog.getOpenFileName(self, "Select Python Interpreter", str(self.project_root), "Python Executable (python.exe);;All Files (*)")
        if python_path:
    ...
    def load_csv_preview(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Open CSV file", str(self.project_root), "CSV Files (*.csv);;All Files (*)")
        if not file_path:
            return
        try:
            preview_lines = []
    ...
    def launch_jupyter_lab(self):
        try:
            subprocess.Popen([self.python_executable, "-m", "jupyter", "lab"], cwd=str(self.project_root), shell=False)
    ...
    def load_ai_model(self):
        if not GPT4ALL_AVAILABLE:
            QMessageBox.warning(self, "AI Backend Missing", "GPT4All is not installed. Install requirements-ai.txt and restart the IDE.")
            return

        model_path, _ = QFileDialog.getOpenFileName(self, "Select GPT4All Model File", str(self.project_root), "Model Files (*.bin *.gguf *.bin.*);;All Files (*)")
        if not model_path:
            return
    ...
    def ask_ai(self):
        ...
        if not prompt:
            ...
            return
        if not GPT4ALL_AVAILABLE:
            self.ai_response.setPlainText("GPT4All package is not installed.\nInstall the AI requirements and restart the IDE.")
            return
        ...
        system_prompt = "You are a Python code assistant for machine learning and data science projects. Answer with code examples, suggestions, or fixes only."
        full_prompt = f"{system_prompt}\n\nCurrent file: {file_name}\nCurrent code:\n{source_snippet}\n\nRequest:\n{prompt}\n"

        try:
            response = self.ai_model.generate(full_prompt, max_tokens=512, temperature=0.2)
